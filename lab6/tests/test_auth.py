...
def test_register_short_password_422(client: TestClient):
    assert client.post("/auth/register", json={**USER_A, "password": "12345"}).status_code == 422
...
def test_login_wrong_password_401(client: TestClient):
    client.post("/auth/register", json=USER_A)
    assert client.post("/auth/login", json={**USER_A, "password": "wrongpass"}).status_code == 401
...
def test_login_two_tokens_differ(client: TestClient):
    client.post("/auth/register", json=USER_A)
    t1 = client.post("/auth/login", json=USER_A).json()["access_token"]
    t2 = client.post("/auth/login", json=USER_A).json()["access_token"]
    assert t1 != t2


# ── POST /auth/refresh ────────────────────────────────────────────────────────

def test_refresh_returns_new_pair(client: TestClient, tokens_a):
    old_access = tokens_a["access_token"]
    data = client.post("/auth/refresh", json={"refresh_token": tokens_a["refresh_token"]}).json()
    assert "access_token" in data
    assert data["access_token"] != old_access


def test_refresh_rotates_token(client: TestClient, tokens_a):
    old_refresh = tokens_a["refresh_token"]
...
def test_refresh_new_token_works(client: TestClient, tokens_a):
    new_tokens = client.post("/auth/refresh", json={"refresh_token": tokens_a["refresh_token"]}).json()
...
def test_logout_invalidates_token(client: TestClient, tokens_a):
