...
def test_create_book_issued_status(client: TestClient):
    data = client.post("/books/", json={**SAMPLE, "status": "issued"}).json()
...
def test_create_book_missing_author_returns_422(client: TestClient):
    book = {k: v for k, v in SAMPLE.items() if k != "author"}
    assert client.post("/books/", json=book).status_code == 422


def test_create_book_missing_year_returns_422(client: TestClient):
    book = {k: v for k, v in SAMPLE.items() if k != "year"}
    assert client.post("/books/", json=book).status_code == 422
...
def test_combined_filter_and_sort(client: TestClient):
    client.post("/books/", json={**SAMPLE, "title": "Z", "author": "Tolkien", "year": 2000})
    client.post("/books/", json={**SAMPLE, "title": "A", "author": "Tolkien", "year": 1990})
    client.post("/books/", json={**SAMPLE, "title": "M", "author": "Rowling", "year": 1995})
