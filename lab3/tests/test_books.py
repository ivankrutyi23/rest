...
def test_create_book_returns_correct_data(client: TestClient):
    ...
    assert data["author"] == SAMPLE["author"]
    assert data["year"] == SAMPLE["year"]
    assert data["status"] == "available"


def test_create_book_generates_unique_uuid(client: TestClient):
    r1 = client.post("/books/", json=SAMPLE).json()
    r2 = client.post("/books/", json=SAMPLE).json()
    assert len(r1["id"]) == 36
    assert r1["id"] != r2["id"]
...
def test_create_book_issued_status(client: TestClient):
    assert client.post("/books/", json={**SAMPLE, "status": "issued"}).json()["status"] == "issued"


def test_create_book_without_description(client: TestClient):
    book = {k: v for k, v in SAMPLE.items() if k != "description"}
...
def test_create_book_missing_author_returns_422(client: TestClient):
    assert client.post("/books/", json={k: v for k, v in SAMPLE.items() if k != "author"}).status_code == 422
...
def test_create_book_empty_title_returns_422(client: TestClient):
    assert client.post("/books/", json={**SAMPLE, "title": ""}).status_code == 422


def test_create_book_empty_author_returns_422(client: TestClient):
    assert client.post("/books/", json={**SAMPLE, "author": ""}).status_code == 422


def test_create_book_year_too_low_returns_422(client: TestClient):
    assert client.post("/books/", json={**SAMPLE, "year": 999}).status_code == 422


def test_create_book_year_too_high_returns_422(client: TestClient):
    assert client.post("/books/", json={**SAMPLE, "year": 2101}).status_code == 422


def test_create_book_invalid_status_returns_422(client: TestClient):
    assert client.post("/books/", json={**SAMPLE, "status": "lost"}).status_code == 422
...
def test_get_all_books_returns_items(client: TestClient):
    client.post("/books/", json=SAMPLE)
    client.post("/books/", json={**SAMPLE, "title": "Book 2"})
    data = client.get("/books/").json()
...
def test_filter_by_status_available(client: TestClient):
    client.post("/books/", json={**SAMPLE, "status": "available"})
    client.post("/books/", json={**SAMPLE, "title": "Issued", "status": "issued"})
...
def test_filter_by_status_issued(client: TestClient):
    client.post("/books/", json={**SAMPLE, "status": "issued"})
    client.post("/books/", json={**SAMPLE, "title": "Available", "status": "available"})
...
def test_filter_by_author_exact(client: TestClient):
    client.post("/books/", json={**SAMPLE, "author": "Tolkien"})
    client.post("/books/", json={**SAMPLE, "title": "HP", "author": "Rowling"})
...
def test_filter_by_author_partial_match(client: TestClient):
    client.post("/books/", json={**SAMPLE, "author": "John Ronald Reuel Tolkien"})
    assert len(client.get("/books/?author=tolkien").json()["items"]) == 1
...
def test_sort_by_title(client: TestClient):
    for title in ["Zebra", "Apple", "Mango"]:
        client.post("/books/", json={**SAMPLE, "title": title})
...
def test_sort_by_year(client: TestClient):
    for year in [2005, 1990, 2020]:
        client.post("/books/", json={**SAMPLE, "title": f"Book {year}", "year": year})
...
def test_cursor_first_page_has_next_cursor(client: TestClient):
    for i in range(5):
        client.post("/books/", json={**SAMPLE, "title": f"Book {i:02d}"})
...
def test_cursor_sort_by_title_traversal(client: TestClient):
    for i in range(6):
        client.post("/books/", json={**SAMPLE, "title": f"Title {i:02d}"})
...
def test_cursor_with_status_filter(client: TestClient):
    for i in range(5):
        client.post("/books/", json={**SAMPLE, "title": f"A{i:02d}", "status": "available"})
    for i in range(3):
        client.post("/books/", json={**SAMPLE, "title": f"I{i:02d}", "status": "issued"})
...
def test_cursor_with_author_filter(client: TestClient):
    for i in range(4):
        client.post("/books/", json={**SAMPLE, "title": f"T{i:02d}", "author": "Tolkien"})
    for i in range(3):
        client.post("/books/", json={**SAMPLE, "title": f"R{i:02d}", "author": "Rowling"})
...
def test_get_book_by_id_returns_200(client: TestClient):
    created = client.post("/books/", json=SAMPLE).json()
...
def test_delete_does_not_affect_other_books(client: TestClient):
    b1 = client.post("/books/", json=SAMPLE).json()
    b2 = client.post("/books/", json={**SAMPLE, "title": "Book 2"}).json()
